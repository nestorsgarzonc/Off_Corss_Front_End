...
import dash_table
#--------------------------------------------
df = pd.read_csv('../../../../Downloads/Off_Corss_Front_End (1)/Off_Corss_Front_End/data/superstore.csv', parse_dates=['Order Date', 'Ship Date'])
df['Order_Month'] = pd.to_datetime(df['Order Date'].map(lambda x: "{}-{}".format(x.year, x.month)))
with open('../../../../Downloads/Off_Corss_Front_End (1)/Off_Corss_Front_End/data/us.json') as geo:
